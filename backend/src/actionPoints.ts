 ...
 * 扣减用单条条件 UPDATE 原子完成（含恢复补点），杜绝「读余额 → 算 → 写绝对值」的
 * 非原子窗口：并发下多次扣减不可能各自读到充足余额后超花。 */
export async function trySpendActionPoints(db: Queryable | null, userId: string, cost: number): Promise<boolean> {
  if (!db) {
    throw new ActionPointError(503, '服务暂不可用（未连接数据库）');
  }
  // 单条原子扣减：先把按恢复周期累积的点补进 current（封顶 max），再扣 cost；
  // 仅当补点后仍 >= cost 才真正扣减并返回，任一步并发下都受行锁保护不会超花。
  const res = await db.query(
    `UPDATE action_points
     SET current = LEAST(max, current + FLOOR(EXTRACT(EPOCH FROM (now() - last_recovered_at)) * 1000 / $2)) - $1,
         last_recovered_at = last_recovered_at
           + (FLOOR(EXTRACT(EPOCH FROM (now() - last_recovered_at)) * 1000 / $2) * interval '1 millisecond')
     WHERE user_id = $3
       AND LEAST(max, current + FLOOR(EXTRACT(EPOCH FROM (now() - last_recovered_at)) * 1000 / $2)) >= $1
     RETURNING current`,
    [cost, AP_RECOVER_MS, userId],
  );
  if (res.rows.length === 0) {
    return false;
  }

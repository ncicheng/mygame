...
export function ActionDeck({ ap, onRecruit, onMarch, marchMode }: ActionDeckProps) {
    ...
    <>
      {ACTIONS.map((action) => (
        <button
          key={action.key}
          type="button"
          className={`act${action.kind ? ' kind' : ''}${action.key === 'march' && marchMode ? ' act-on' : ''}`}
          disabled={action.cost > ap.current}
          onClick={() => {
            if (action.key === 'recruit') {
              onRecruit();
            } else if (action.key === 'march') {
              setHint(marchMode ? '行动点每 10 分钟恢复 1 点' : '点击我方部队选择行军，再点目标格下达命令');
              onMarch();
            } else {
              setHint(action.hint);
            }
